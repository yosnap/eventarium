...
/**
 * Diálogo de selección de medios: pestañas Subir (dropzone + selector de
 * fichero), URL (importar por dirección) y Biblioteca (rejilla de imágenes
 * existentes que quien llama facilite). Referencia: el media picker de
 * Coetools (pestañas, valor plano por URL, Esc/fondo para cerrar), sobre el
 * `Dialog` del catálogo.
 *
 * Dos salidas excluyentes según la pestaña:
...
@Component({
  ...
  imports: [TranslocoDirective, Dialog, Button, Input],
  template: `
    <ng-container *transloco="let t">
      <app-dialog (cerrado)="cerrado.emit()">
        ...
        @switch (pestana()) {
          ...
          @case ('biblioteca') {
            @if (biblioteca().length === 0) {
              ...
              <div class="rejilla" role="listbox" [attr.aria-label]="titulo()">
