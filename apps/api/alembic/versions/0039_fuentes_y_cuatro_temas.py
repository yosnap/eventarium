...
def upgrade() -> None:
    ...
    if fila is not None:
        ...
        bind.execute(
            sa.text(
                "UPDATE theme_templates SET tokens = CAST(:tokens AS jsonb) WHERE key = 'por-defecto'"
...
def downgrade() -> None:
    ...
    for plantilla in _PLANTILLAS_NUEVAS:
        bind.execute(
            sa.text("DELETE FROM theme_templates WHERE key = :key").bindparams(key=plantilla["key"])
        )
    fila = bind.execute(
        sa.text("SELECT tokens FROM theme_templates WHERE key = 'por-defecto'")
    ).scalar_one_or_none()
