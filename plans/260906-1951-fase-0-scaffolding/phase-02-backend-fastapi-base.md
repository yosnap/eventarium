...
- **Auth (hallazgo #9)**: access token JWT (15 min) en respuesta JSON; refresh token opaco (7 días) en cookie `HttpOnly; Secure; SameSite=Lax; Path=/api/v1/auth` con `Domain` derivado del host de la organización; rotación en cada uso; familia de tokens en Redis con TTL igual a la caducidad; reutilización de un refresh rotado invalida la familia. Redis no disponible → `503` en refresh/logout (fail-closed).
...
9. `main.py`: `create_app()`, CORS con orígenes = dominios registrados (`organization_domains`) + `CORS_ORIGINS` de desarrollo, handlers `problem+json` sin trazas de BD, rate limiting.
...
- Cookie `Domain` con dominios distintos por organización → derivar del host resuelto; test con dos hosts.
